...
type PersonLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*api.Person, []error)
...
func (l *PersonLoader) Load(key string) (*api.Person, error) {
...
func (l *PersonLoader) LoadThunk(key string) func() (*api.Person, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*api.Person, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *PersonLoader) LoadAll(keys []string) ([]*api.Person, []error) {
	results := make([]func() (*api.Person, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	persons := make([]*api.Person, len(keys))
...
func (l *PersonLoader) Prime(key string, value *api.Person) bool {
	l.mu.Lock()
	var found bool
...
func (l *PersonLoader) unsafeSet(key string, value *api.Person) {
	if l.cache == nil {
		l.cache = map[string]*api.Person{}
	}
	l.cache[key] = value
}
